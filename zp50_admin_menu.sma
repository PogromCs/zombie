...
show_menu_admin(id)
{
	...
	if (LibraryExists(LIBRARY_SNIPER, LibType_Library) && (userflags & read_flags(g_access_make_sniper)))
		len += formatex(menu[len], charsmax(menu) - len, "\r3.\w %L^n", id, "MENU_ADMIN5")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d3. %L^n", id, "MENU_ADMIN5")

	//6 Lowca comand
	if (LibraryExists(LIBRARY_LOWCA, LibType_Library) && (userflags & read_flags(g_access_make_lowca)))
		len += formatex(menu[len], charsmax(menu) - len, "\r3.\w %L^n", id, "MENU_ADMIN6")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d3. %L^n", id, "MENU_ADMIN6")
	
	// 4. Respawn command
	if (userflags & read_flags(g_access_respawn_players))
		len += formatex(menu[len], charsmax(menu) - len, "\r5.\w %L^n", id, "MENU_ADMIN7")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d5. %L^n", id, "MENU_ADMIN7")
	...
	if (userflags & read_flags(g_access_start_game_mode))
		...
		len += formatex(menu[len], charsmax(menu) - len, "\d6. %L^n", id, "MENU_ADMIN_START_GAME_MODE")
...
show_menu_player_list(id)
{
	...
	switch (PL_ACTION)
	{
		...
		case ACTION_MAKE_ASSASSIN: formatex(menu, charsmax(menu), "%L\r", id, "MENU_ADMIN4")
		case ACTION_MAKE_SNIPER: formatex(menu, charsmax(menu), "%L\r", id, "MENU_ADMIN5")
		case ACTION_MAKE_LOWCA: formatex(menu, charsmax(menu), "%L\r", id, "MENU_ADMIN6")
	...
	for (player = 0; player <= g_MaxPlayers; player++)
	{
		...
		switch (PL_ACTION)
		{
			...
			case ACTION_MAKE_ASSASSIN: // Survivor command
			{
				...
				{
					if (zp_core_is_zombie(player))
						formatex(menu, charsmax(menu), "%s \r[%L]", player_name, id, (LibraryExists(LIBRARY_ASSASSIN, LibType_Library) && zp_class_assassin_get(player)) ? "CLASS_ASSASSIN" : "CLASS_ZOMBIE")
					else
						formatex(menu, charsmax(menu), "%s \y[%L]", player_name, id, (LibraryExists(LIBRARY_SNIPER, LibType_Library) && zp_class_sniper_get(player)) ? "CLASS_SNIPER" : "CLASS_HUMAN")
				}
				else
					formatex(menu, charsmax(menu), "\d%s [%L]", player_name, id, zp_core_is_zombie(player) ? (LibraryExists(LIBRARY_ASSASSIN, LibType_Library) && zp_class_assassin_get(player)) ? "CLASS_ASSASSIN" : "CLASS_ZOMBIE" : (LibraryExists(LIBRARY_SNIPER, LibType_Library) && zp_class_sniper_get(player)) ? "CLASS_SNIPER" : "CLASS_HUMAN")
			}
			...
			case ACTION_MAKE_LOWCA: // Survivor command
			{
				if (LibraryExists(LIBRARY_LOWCA, LibType_Library) && (userflags & read_flags(g_access_make_lowca)) && is_user_alive(player) && !zp_class_lowca_get(player))
				{
					if (!zp_core_is_zombie(player))
						formatex(menu, charsmax(menu), "%s \r[%L]", player_name, id, (LibraryExists(LIBRARY_ASSASSIN, LibType_Library) && zp_class_assassin_get(player)) ? "CLASS_ASSASSIN" : "CLASS_ZOMBIE")
					else
						formatex(menu, charsmax(menu), "%s \y[%L]", player_name, id, (LibraryExists(LIBRARY_LOWCA, LibType_Library) && zp_class_lowca_get(player)) ? "CLASS_LOWCA" : "CLASS_HUMAN")
				}
				else
					formatex(menu, charsmax(menu), "\d%s [%L]", player_name, id, zp_core_is_zombie(player) ? (LibraryExists(LIBRARY_ASSASSIN, LibType_Library) && zp_class_assassin_get(player)) ? "CLASS_ASSASSIN" : "CLASS_ZOMBIE" : (LibraryExists(LIBRARY_SNIPER, LibType_Library) && zp_class_lowca_get(player)) ? "CLASS_LOWCA" : "CLASS_HUMAN")
			}
...
public menu_admin(id, key)
{
	...
	switch (key)
	{
		...
		case ACTION_MAKE_ASSASSIN: // Survivor command
		{
			if (LibraryExists(LIBRARY_SURVIVOR, LibType_Library) && (userflags & read_flags(g_access_make_assassin)))
			{
				// Show player list for admin to pick a target
...
public menu_player_list(id, menuid, item)
{
	...
	if (is_user_connected(player))
	{
		...
		switch (PL_ACTION)
		{
			...
			{
				if (LibraryExists(LIBRARY_SURVIVOR, LibType_Library) && (userflags & read_flags(g_access_make_survivor)) && is_user_alive(player) && !zp_class_survivor_get(player))
					zp_admin_commands_lowca(id, player)
				else
					zp_colored_print(id, "%L", id, "CMD_NOT")
			}
			case ACTION_RESPAWN_PLAYER: // Respawn command
