...
#define LIBRARY_LOWCA "zp50_class_lowca"
...
new g_access_make_lowca[ACCESSFLAG_MAX_LENGTH] = "d"
new g_access_make_sniper[ACCESSFLAG_MAX_LENGTH] = "d"
new g_access_start_game_mode[ACCESSFLAG_MAX_LENGTH] = "d"

new g_MaxPlayers
new g_GameModeInfectionID, g_GameModeNemesisID, g_GameModeSurvivorID,g_GameModeLowcaID,g_GameModeAssassinID,g_GameModeSniperID
...
public plugin_init()
{
	...
	if (LibraryExists(LIBRARY_LOWCA, LibType_Library))
		register_concmd("zp_lowca", "cmd_lowca", _, "<target> - Turn someone into a Hunter", 0)
	if (LibraryExists(LIBRARY_ASSASSIN, LibType_Library))
		register_concmd("zp_assassin", "cmd_assasin", _, "<target> - Turn someone into a Assassin", 0)
	if (LibraryExists(LIBRARY_ASSASSIN, LibType_Library))
		register_concmd("zp_sniper", "cmd_sniper", _, "<target> - Turn someone into a Assassin", 0)
...
public plugin_precache()
{
	...
	if (!amx_load_setting_string(ZP_SETTINGS_FILE, "Access Flags", "MAKE LOWCA", g_access_make_lowca, charsmax(g_access_make_lowca)))
		amx_save_setting_string(ZP_SETTINGS_FILE, "Access Flags", "MAKE LOWCA", g_access_make_lowca)
...
public plugin_natives()
{
	...
	register_native("zp_admin_commands_assassin", "native_admin_commands_sniper")
	register_native("zp_admin_commands_lowca", "native_admin_commands_lowca")
...
{
	if (equal(module, LIBRARY_NEMESIS) || equal(module, LIBRARY_SURVIVOR)|| equal(module, LIBRARY_LOWCA)|| equal(module, LIBRARY_ASSASSIN))
...
public plugin_cfg()
{
	...
	g_GameModeLowcaID = zp_gamemodes_get_id("Lowca Mode")
...
public native_admin_commands_lowca(plugin_id, num_params)
{
	new id_admin = get_param(1)
...
public cmd_lowca(id, level, cid)
{
	...
	if (!allowed_respawn(id))
	{
		new player_name[32]
		get_user_name(player, player_name, charsmax(player_name))
		client_print(id, print_console, "[ZP] %L (%s).", id, "ALREADY_LOWCA", player_name)
		return PLUGIN_HANDLED;
	}
	
	command_lowca(id, player)
	return PLUGIN_HANDLED;
}
...
command_zombie(id, player)
{
	...
	if (get_pcvar_num(cvar_log_admin_commands))
	{
		...
		log_amx("ADMIN %s <%s><%s> - %s %L (Players: %d)", admin_name, authid, ip, player_name, LANG_SERVER, "CMD_INFECT", GetPlayingCount())
	}
}
...
command_human(id, player)
{
	...
	if (get_pcvar_num(cvar_log_admin_commands))
	{
		...
		log_amx("ADMIN %s <%s><%s> - %s %L (Players: %d)", admin_name, authid, ip, player_name, LANG_SERVER, "CMD_DISINFECT", GetPlayingCount())
	}
}
...
command_nemesis(id, player)
{
	...
	if (get_pcvar_num(cvar_log_admin_commands))
	{
		...
		log_amx("ADMIN %s <%s><%s> - %s %L (Players: %d)", admin_name, authid, ip, player_name, LANG_SERVER, "CMD_NEMESIS", GetPlayingCount())
	}
}
command_assassin(id, player)
{
	...
	if (get_pcvar_num(cvar_log_admin_commands))
	{
		...
		log_amx("ADMIN %s <%s><%s> - %s %L (Players: %d)", admin_name, authid, ip, player_name, LANG_SERVER, "CMD_ASSASSIN", GetPlayingCount())
	}
}
command_lowca(id, player)
{
	...
	{
		// Nemesis mode disabled
		if (g_GameModeLowcaID == ZP_INVALID_GAME_MODE)
		{
			zp_colored_print(id, "%L", id, "CMD_ONLY_AFTER_GAME_MODE")
			return;
		}
		...
		// Start nemesis game mode with this target player
		if (!zp_gamemodes_start(g_GameModeLowcaID, player))
		{
			zp_colored_print(id, "%L", id, "GAME_MODE_CANT_START")
			return;
		}
	...
	if (cvar_amx_show_activity)
	{
		switch (get_pcvar_num(cvar_amx_show_activity))
		{
			case 1: zp_colored_print(0, "ADMIN - %s %L", player_name, LANG_PLAYER, "CMD_LOWCA")
			case 2: zp_colored_print(0, "ADMIN %s - %s %L", admin_name, player_name, LANG_PLAYER, "CMD_LOWCA")
		}
	}
	...
	if (get_pcvar_num(cvar_log_admin_commands))
	{
		...
		get_user_authid(id, authid, charsmax(authid))
		get_user_ip(id, ip, charsmax(ip), 1)
		zp_log("ADMIN %s <%s><%s> - %s %L (Players: %d)", admin_name, authid, ip, player_name, LANG_SERVER, "CMD_LOWCA", GetPlayingCount())
		log_amx("ADMIN %s <%s><%s> - %s %L (Players: %d)", admin_name, authid, ip, player_name, LANG_SERVER, "CMD_LOWCA", GetPlayingCount())
	}
}
command_sniper(id, player)
{
	...
	if (get_pcvar_num(cvar_log_admin_commands))
	{
		...
		log_amx("ADMIN %s <%s><%s> - %s %L (Players: %d)", admin_name, authid, ip, player_name, LANG_SERVER, "CMD_SNIPER", GetPlayingCount())
	}
}
...
command_survivor(id, player)
{
	...
	if (get_pcvar_num(cvar_log_admin_commands))
	{
		...
		log_amx("ADMIN %s <%s><%s> - %s %L (Players: %d)", admin_name, authid, ip, player_name, LANG_SERVER, "CMD_SURVIVAL", GetPlayingCount())
	}
}
...
command_respawn(id, player)
{
	...
	if (get_pcvar_num(cvar_log_admin_commands))
	{
		...
		log_amx("ADMIN %s <%s><%s> - %s %L (Players: %d)", admin_name, authid, ip, player_name, LANG_SERVER, "CMD_RESPAWN", GetPlayingCount())
	}
}
...
command_start_mode(id, game_mode_id)
{
	...
	if (get_pcvar_num(cvar_log_admin_commands))
	{
		...
		log_amx("ADMIN %s <%s><%s> - %L: %s (Players: %d)", admin_name, authid, ip, LANG_SERVER, "CMD_START_GAME_MODE", mode_name, GetPlayingCount())
	}
}
